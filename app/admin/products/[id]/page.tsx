    ...
    <main className="admin-wrap">
      ...
      <section className="panel">
        ...
        >
          ...
          <label className="full"><span>Description longue</span><textarea name="longDescription" defaultValue={product.longDescription} required /></label>
          ...
          {product.image ? (
            <div className="full">
              <img src={product.image} alt={product.title} className="product-image" style={{ maxWidth: 220 }} />
            </div>
          ) : null}

          <label><span>Type d’offre</span><select name="offerType" defaultValue={product.offerType}><option value="product">Produit</option><option value="donation">Don</option><option value="campaign">Collecte</option><option value="participation">Participation</option></select></label>
          <label><span>Mode de prix</span><select name="pricingMode" defaultValue={product.pricingMode}><option value="fixed">Prix fixe</option><option value="flexible">Montant libre</option></select></label>
          <label><span>Prix fixe (centimes)</span><input name="fixedPrice" type="number" defaultValue={product.fixedPrice} /></label>
          <label><span>Minimum (centimes)</span><input name="minimumAmount" type="number" defaultValue={product.minimumAmount} /></label>
          <label><span>Montant suggéré (centimes)</span><input name="suggestedAmount" type="number" defaultValue={product.suggestedAmount} /></label>
          <label><span>Catégorie</span><input name="category" defaultValue={product.category} /></label>
          <label><span>SKU</span><input name="sku" defaultValue={product.sku} /></label>
