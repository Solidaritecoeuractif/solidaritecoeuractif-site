...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Les informations du contact principal sont obligatoires." },
        { status: 400 }
      );
    }
    ...
    if (!isValidEmail(payer.email)) {
      return NextResponse.json(
        { error: "L’adresse email du contact principal est invalide." },
        { status: 400 }
      );
    }
    ...
    if (totalAmount <= 0) {
      return NextResponse.json(
        {
          error:
            "Le montant total est à 0 €. Le paiement Stripe ne peut être lancé que pour une inscription payante.",
        },
        { status: 400 }
      );
    }
    ...
    const session = await stripe().checkout.sessions.create({
      ...
      success_url: `${baseUrl}/evenements/${event.slug}?paiement=success&reference=${reference}`,
      cancel_url: `${baseUrl}/evenements/${event.slug}?paiement=cancel&reference=${reference}`,
      ...
      metadata: {
        ...
        extraDonationAmount: String(finalExtraDonationAmount),
    ...
    await storage.updateTicketingOrder(reference, order);
    ...
    console.error("Erreur checkout billetterie", error);

    const message =
      error instanceof Error
        ? error.message
        : "Impossible de lancer le paiement de cette inscription.";
