...
function rowToProduct(row: any): Product {
  return {
    ...
    minimumAmountOutreMer: row.minimum_amount_outre_mer ?? undefined,
    minimumAmountInternational: row.minimum_amount_international ?? undefined,
...
function rowToOrder(row: any, items: any[]): Order {
  return {
    ...
    emailSentAt: row.email_sent_at ? row.email_sent_at.toISOString() : undefined,
    paymentReceiptSentAt: row.payment_receipt_sent_at
      ? row.payment_receipt_sent_at.toISOString()
      : undefined,
  };
}
    ...
    try {
      await client.query(
        `insert into products (
          ...
          fixed_price,
          minimum_amount,
          minimum_amount_outre_mer,
          minimum_amount_international,
          suggested_amount,
          is_active,
          is_physical,
          requires_shipping,
        ...
        [
          ...
          product.minimumAmountOutreMer ?? null,
          product.minimumAmountInternational ?? null,
      ...
      await client.query(
        `update products
         set slug = $2,
             ...
             fixed_price = $10,
             minimum_amount = $11,
             minimum_amount_outre_mer = $12,
             minimum_amount_international = $13,
             suggested_amount = $14,
             is_active = $15,
             is_physical = $16,
             requires_shipping = $17,
             shipping_fee_amount = $18,
             is_featured = $19,
             max_quantity = $20,
             stock = $21,
             sku = $22,
  ...
  async getOrders() {
    ...
    try {
      ...
      return ordersRes.rows.map((row) =>
        rowToOrder(
          row,
          itemsRes.rows.filter((item) => item.order_id === row.id)
        )
      );
    } finally {
      client.release();
    ...
    try {
      ...
      await client.query(
        ...
        [
          ...
          order.paymentReceiptSentAt ?? null,
        ]
      );
